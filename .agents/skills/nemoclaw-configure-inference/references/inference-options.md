...
|--------|-------------|----------------|
| NVIDIA Endpoints | Routes to models hosted on [build.nvidia.com](https://build.nvidia.com). You can also enter any model ID from the catalog. Set `NVIDIA_API_KEY`. | Nemotron 3 Ultra 550B, Llama 4 Maverick, DeepSeek v4 Pro, Qwen 3.5 397B, Kimi K2.6, GPT-OSS 120B, Gemma 4 31B, Mistral Small 4 119B (auto-curated daily — see OpenWebUI model picker (use the `nemoclaw-manage-operations` skill)) |
...
| Local Ollama | Routes to a local Ollama instance on `localhost:11434`. NemoClaw detects installed models, offers starter models if none are present, pulls and warms the selected model, and validates it. | Selected during onboarding. For more information, refer to Use a Local Inference Server (use the `nemoclaw-configure-inference` skill). |
...
For setup instructions, refer to Use a Local Inference Server (use the `nemoclaw-configure-inference` skill).
...
- Use a Local Inference Server (use the `nemoclaw-configure-inference` skill) for Ollama, vLLM, NIM, and compatible-endpoint setup details.
- Switch Inference Models (use the `nemoclaw-configure-inference` skill) for changing the model at runtime without re-onboarding.
