...
Refer to Customize the Network Policy (use the `nemoclaw-manage-policy` skill) for policy update workflows.
...
- **Load [references/providers.md](references/providers.md)** when the user asks why a task is slow/poor or how to upgrade a flow. Reference for which model serves which task and when premium (Anthropic) routing is available.
...
- `nemoclaw-workspace` — Refer to Workspace Files (use the `nemoclaw-workspace` skill) to understand per-agent workspace directories
