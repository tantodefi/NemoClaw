...
For background on how the layers fit together, refer to How It Works (use the `nemoclaw-overview` skill).
...
For additional protection, pass `--cap-drop=ALL` with `docker run` or Compose (see Sandbox Hardening (use the `nemoclaw-deploy-remote` skill)).
...
- Network Policies (use the `nemoclaw-reference` skill) for the full baseline policy reference.
- Customize the Network Policy (use the `nemoclaw-manage-policy` skill) for static and dynamic policy changes.
- Approve or Deny Network Requests (use the `nemoclaw-manage-policy` skill) for the operator approval flow.
- Sandbox Hardening (use the `nemoclaw-deploy-remote` skill) for container-level security measures.
- Inference Options (use the `nemoclaw-configure-inference` skill) for provider configuration details.
- How It Works (use the `nemoclaw-overview` skill) for the protection layer architecture.
