...
> **Warning:** **`/sandbox` is on the pod's writable container layer — there is no PVC
...
> nemoclaw-blueprint StatefulSet. See Backup and Restore (use the `nemoclaw-workspace` skill)
...
- Set Up Task-Specific Sub-Agents (use the `nemoclaw-configure-inference` skill)
- Backup and Restore workspace files (use the `nemoclaw-workspace` skill)
- Commands reference (use the `nemoclaw-reference` skill)
