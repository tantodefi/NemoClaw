...
For repository layout, file paths, and deeper diagrams, see Architecture (use the `nemoclaw-reference` skill).
...
For details on the baseline rules, refer to Network Policies (use the `nemoclaw-reference` skill). For container-level hardening, refer to Sandbox Hardening (use the `nemoclaw-deploy-remote` skill).

## Next Steps

- Read Ecosystem (use the `nemoclaw-overview` skill) for stack-level relationships and NemoClaw versus OpenShell-only paths.
- Follow the Quickstart (use the `nemoclaw-get-started` skill) to launch your first sandbox.
- Refer to the Architecture (use the `nemoclaw-reference` skill) for the full technical structure, including file layouts and the blueprint lifecycle.
- Refer to Inference Options (use the `nemoclaw-configure-inference` skill) for detailed provider configuration.
