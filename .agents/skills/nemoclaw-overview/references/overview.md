...
| Routed inference           | Model traffic is routed through the OpenShell gateway to your selected provider, transparent to the agent. You can switch providers or models. Refer to Inference Options (use the `nemoclaw-configure-inference` skill).          |
...
- Ecosystem (use the `nemoclaw-overview` skill) to understand how OpenClaw, OpenShell, and NemoClaw relate in the wider stack, and when to use NemoClaw versus OpenShell.
- How It Works (use the `nemoclaw-overview` skill) to understand how NemoClaw works internally: plugin, blueprint, sandbox lifecycle.
- Quickstart (use the `nemoclaw-get-started` skill) to install NemoClaw and run your first agent.
- Switch Inference Providers (use the `nemoclaw-configure-inference` skill) to configure the inference provider.
- Approve or Deny Network Requests (use the `nemoclaw-manage-policy` skill) to manage egress approvals.
- Deploy to a Remote GPU Instance (use the `nemoclaw-deploy-remote` skill) for persistent operation.
- Monitor Sandbox Activity (use the `nemoclaw-monitor-sandbox` skill) to observe agent behavior.
