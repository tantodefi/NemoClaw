...
For details, refer to Commands (use the `nemoclaw-reference` skill).
...
For a full first-time flow, refer to Quickstart (use the `nemoclaw-get-started` skill).

## Confirm Delivery

After the sandbox is running, send a message to your bot in Telegram.
If something fails, use `openshell term` on the host, check gateway logs, and verify network policy allows the Telegram API (see Customize the Network Policy (use the `nemoclaw-manage-policy` skill) and the `telegram` preset).
...
- Deploy NemoClaw to a Remote GPU Instance (use the `nemoclaw-deploy-remote` skill) for remote deployment with messaging.
- Architecture (use the `nemoclaw-reference` skill) for how providers, the gateway, and the sandbox fit together.
- Commands (use the `nemoclaw-reference` skill) for `tunnel start`, `tunnel stop`, `channels start`, `channels stop`, and `status`.
