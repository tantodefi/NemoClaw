...
For example, see Network Policies (use the `nemoclaw-reference` skill) for
policy concepts and Customize Network Policy (use the `nemoclaw-manage-policy` skill)
...
- Review Sandbox Hardening (use the `nemoclaw-deploy-remote` skill) before adding plugin code to a
  shared or long-lived sandbox.
- Review Network Policies (use the `nemoclaw-reference` skill) to plan plugin
  egress rules.
- Follow Customize Network Policy (use the `nemoclaw-manage-policy` skill)
