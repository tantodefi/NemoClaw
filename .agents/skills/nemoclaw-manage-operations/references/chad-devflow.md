...
| `chad-dump-logs` | Tarball of recent gateway/cron/sub-agent logs. See Log Locations (use the `nemoclaw-manage-operations` skill). |
...
For the full skill catalog see chad-skills.md (use the `nemoclaw-manage-operations` skill). For the
autonomy loops Chad runs on top of these skills see
chad-autonomy.md (use the `nemoclaw-manage-operations` skill).
...
See Workspace Files §Sectioned Manifest (use the `nemoclaw-workspace` skill) for the complete manifest format and Backup Policy §1.2 (use the `nemoclaw-resources` skill) for the rationale behind exclusions.
...
| Cron job calls a `chad-*` wrapper that crashes / silently no-ops | One of the five tracked wrapper bugs in `wrapper-bugs.md` (use the `nemoclaw-manage-operations` skill). Sandbox-writable patched copies live at `/sandbox/.openclaw-data/bin/chad-{budget,issue-triage,issue-triage-cron,mail-check,email-check-cron}` and are persisted by the workspace backup. Cron payloads are pointed at them via absolute path (the openclaw runtime currently appends `/sandbox/.openclaw-data/bin` to PATH instead of prepending, so PATH-based shadowing alone doesn't take). |

## Next Steps

- Backup and Restore (use the `nemoclaw-workspace` skill) — the canonical persistence guide
- Workspace Files (use the `nemoclaw-workspace` skill) — what each file does and the sectioned manifest format
- Backup Policy (use the `nemoclaw-resources` skill) — full inventory across all four state layers
- Open WebUI Front-End (use the `nemoclaw-manage-operations` skill) — chat UI exposed via Cloudflare Tunnel + Access
- Workflow Scenarios (use the `nemoclaw-manage-operations` skill) — named email scenarios + regression spec
- Log Locations (use the `nemoclaw-manage-operations` skill) — where each log stream lives
- Chad Autonomy Loops (use the `nemoclaw-manage-operations` skill) — the six self-driving loops, the wrappers that close them, the sub-agent orchestration contract, and the maintenance crons behind them
- Chad Skills Catalog (use the `nemoclaw-manage-operations` skill) — all 48 registered skills grouped by source
- Wrapper Bugs (use the `nemoclaw-manage-operations` skill) — five tracked argv-vs-env / path bugs in `/usr/local/bin/chad-*` with shim-and-repoint workarounds in place
- Autonomous Experiment Lifecycle (use the `nemoclaw-manage-operations` skill) — Chad's nightly observe → evaluate → design loop with full autonomy + retire-on-regression. `chad-experiment` CLI + `chad-experiment-cron` deterministic driver (`nightly-experiments` cron) + sibling skill at `/sandbox/.openclaw-data/skills/chad-experiment/`
