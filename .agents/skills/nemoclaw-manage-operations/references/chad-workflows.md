...
- **Out of scope here:** outbound-initiated work (cron-driven backups, gbrain dream, self-improve). Those are documented in chad-devflow.md (use the `nemoclaw-manage-operations` skill) §Cron pipeline.
...
- chad-devflow.md (use the `nemoclaw-manage-operations` skill) — wrapper catalog.
