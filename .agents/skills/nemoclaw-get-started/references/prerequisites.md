...
See the troubleshooting guide (use the `nemoclaw-reference` skill) for the override knobs and a manual `daemon.json` alternative.
:::
...
- Prepare Windows for NemoClaw (use the `nemoclaw-get-started` skill) if you are using Windows.
- Quickstart (use the `nemoclaw-get-started` skill) to install NemoClaw and launch your first sandbox.
