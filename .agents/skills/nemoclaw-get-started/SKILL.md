...
> **Note:** Make sure you have completed reviewing the Prerequisites (use the `nemoclaw-get-started` skill) before following this guide.
...
For the full list of providers and validation behavior, refer to Inference Options (use the `nemoclaw-configure-inference` skill).
...
For setup details, including GPU recommendations and starter model choices, refer to Use a Local Inference Server (use the `nemoclaw-configure-inference` skill).
...
For setup, refer to Use a Local Inference Server (use the `nemoclaw-configure-inference` skill).
:::
...
For full details on port conflicts and overrides, refer to Port already in use (use the `nemoclaw-reference` skill).

### Open the UI from a Remote Host

If NemoClaw is running on a remote GPU instance and you want to open the UI from a laptop, refer to Remote Dashboard Access (use the `nemoclaw-deploy-remote` skill). Set `CHAT_UI_URL` to the origin the browser uses before running onboard, so the gateway's CORS allowlist accepts the remote browser.
...
Refer to Switch inference providers (use the `nemoclaw-configure-inference` skill) for provider-specific model IDs and API compatibility notes.
...
The credentials command is documented in full at `nemoclaw credentials reset <PROVIDER>` (use the `nemoclaw-reference` skill).
...
Rebuild preserves the mounted workspace and registered policies while recreating the container. Refer to `nemoclaw <name> rebuild` (use the `nemoclaw-reference` skill) for flag details.
...
Refer to `nemoclaw <name> policy-add` (use the `nemoclaw-reference` skill) for usage details and flags.
...
Before it onboards anything, the installer calls `nemoclaw backup-all` (use the `nemoclaw-reference` skill) automatically, storing a snapshot of each running sandbox in `~/.nemoclaw/rebuild-backups/` as a safety net.
...
Each rebuild destroys the existing container and creates a new one. NemoClaw protects your data through the same backup-and-restore flow as `nemoclaw <name> rebuild` (use the `nemoclaw-reference` skill):
...
See Backup and Restore (use the `nemoclaw-workspace` skill) for the full list of state-preservation guarantees, snapshot retention, and instructions for manual backups when the auto-flow is not enough.
...
For a full comparison of the two forms — what they fetch, what they trust, and when to prefer each — see `nemoclaw uninstall` vs. the hosted `uninstall.sh` (use the `nemoclaw-reference` skill).
...
- `nemoclaw-configure-inference` — Switch inference providers (use the `nemoclaw-configure-inference` skill) to use a different model or endpoint
- `nemoclaw-manage-policy` — Approve or deny network requests (use the `nemoclaw-manage-policy` skill) when the agent tries to reach external hosts
- `nemoclaw-deploy-remote` — Deploy to a remote GPU instance (use the `nemoclaw-deploy-remote` skill) for always-on operation
- `nemoclaw-monitor-sandbox` — Monitor sandbox activity (use the `nemoclaw-monitor-sandbox` skill) through the OpenShell TUI
- `nemoclaw-reference` — Consult the troubleshooting guide (use the `nemoclaw-reference` skill) for common error messages and resolution steps
