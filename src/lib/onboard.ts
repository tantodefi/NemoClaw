...
async function createSandbox(
  ...
  const conflictCheckChannels: string[] = Array.isArray(enabledChannels)
    ? enabledChannels.filter((name) => {
        ...
        return def && def.envKey ? !!getMessagingToken(def.envKey) : false;
      })
    : [];
  ...
  // Downstream: inject sandbox tool credentials explicitly (buildSubprocessEnv strips all secrets)
  const githubToken = getCredential("GITHUB_TOKEN") || process.env.GITHUB_TOKEN;
  if (githubToken) sandboxEnv.GITHUB_TOKEN = githubToken;
  const protonUser = getCredential("PROTON_USERNAME");
  if (protonUser) sandboxEnv.PROTON_USERNAME = protonUser;
  const protonPass = getCredential("PROTON_PASSWORD");
  if (protonPass) sandboxEnv.PROTON_PASSWORD = protonPass;
...
async function setupMessagingChannels(): Promise<string[]> {
  ...
  for (const name of selected) {
    ...
    if (ch.credType === "creds-mount") {
      ...
      // Malformed channel: not creds-mount but no envKey either.
      console.log(`  Skipped ${ch.name} (no auth method configured)`);
