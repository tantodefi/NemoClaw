---
title:
  page: "NemoClaw Inference Providers and Per-Task Routing"
  nav: "Providers and Routing"
description:
  main: "Catalog of inference providers wired into NemoClaw, per-task model assignments, and how to opt into the premium Anthropic endpoint for specific flows."
  agent: "Reference for which model serves which task and when premium (Anthropic) routing is available. Use when the user asks why a task is slow/poor or how to upgrade a flow."
keywords: ["nemoclaw providers", "task profiles", "claude sonnet", "premium inference", "anthropic"]
topics: ["generative_ai", "ai_agents", "inference_routing"]
tags: ["openclaw", "openshell", "nemoclaw", "anthropic", "inference"]
content:
  type: reference
...
This page catalogs every provider NemoClaw can route through, names the model that serves each task by default, and explains how to opt into the premium (Anthropic) endpoint for flows that benefit from it. See also [Inference Options](inference-options.md) for what the onboard wizard exposes.
...
Cron ticks with no inbound trigger have no AuthContext, so premium calls fail closed. See [Log Locations — Premium ledger](../operations/log-locations.md#premium-anthropic-ledger) for where the audit trail lives.
